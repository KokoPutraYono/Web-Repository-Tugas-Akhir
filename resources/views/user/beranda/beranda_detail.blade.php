    ...
    <div class="w-full h-screen md:w-3/4 mx-auto py-4 px-4 md:px-10 md:py-6 border-l-2 border-r-2 border-black">
        ...
        <div id="dataFile" class="hidden pt-4 px-4 md:pt-8 text-sm md:text-lg font-bold">
            ...
            <div class="flex my-2 md:my-4 items-center">
                ...
                <a href="{{ route('beranda-detail-file', ['nama_file' => 'bab1', $showDetail->_id]) }}" class="ml-2">Bab 1</a>
            </div>
            @endif
            <!-- file bab2 -->
            @if($bab2 !== null)
            <div class="flex my-2 md:my-4 items-center">
                <img src="{{ asset('images/icon_TA/pdf.png') }}" class="w-7 h-8 md:w-9 md:h-10" alt="PDF logo">
                <a href="{{ route('beranda-detail-file', ['nama_file' => 'bab2', $showDetail->_id]) }}" class="ml-2">Bab 2</a>
            </div>
            @endif
            <!-- file bab3 -->
            @if($bab3 !== null)
            <div class="flex my-2 md:my-4 items-center">
                <img src="{{ asset('images/icon_TA/pdf.png') }}" class="w-7 h-8 md:w-9 md:h-10" alt="PDF logo">
                <a href="{{ route('beranda-detail-file', ['nama_file' => 'bab3', $showDetail->_id]) }}" class="ml-2">Bab 3</a>
            </div>
            @endif
            <!-- file bab4 -->
            @if($bab4 !== null)
            <div class="flex my-2 md:my-4 items-center">
                <img src="{{ asset('images/icon_TA/pdf.png') }}" class="w-7 h-8 md:w-9 md:h-10" alt="PDF logo">
                <a href="{{ route('beranda-detail-file', ['nama_file' => 'bab4', $showDetail->_id]) }}" class="ml-2">Bab 4</a>
            </div>
            @endif
            <!-- file bab5 -->
            @if($bab5 !== null)
            <div class="flex my-2 md:my-4 items-center">
                <img src="{{ asset('images/icon_TA/pdf.png') }}" class="w-7 h-8 md:w-9 md:h-10" alt="PDF logo">
                <a href="{{ route('beranda-detail-file', ['nama_file' => 'bab5', $showDetail->_id]) }}" class="ml-2">Bab 5</a>
            </div>
            @endif
        </div>
