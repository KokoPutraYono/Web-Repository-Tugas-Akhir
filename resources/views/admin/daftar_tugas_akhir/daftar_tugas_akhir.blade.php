...
@section('content')
    <!-- search input -->
    <form action="{{ route('daftar-tugas-akhir') }}" method="GET" class="flex items-center justify-center border-0 pt-5 pb-8">  
    ...
    @foreach($showTA as $show)
        ...
        <div class="bg-white shadow-lg rounded-lg p-2 m-2 transform hover:-translate-y-2 transition duration-300 w-full md:w-1/2 lg:w-1/3 xl:w-1/4 dark:bg-gray-800 dark:text-white">
            ...
            @if(auth()->user()->user_status == 'super admin' && auth()->user()->jabatan == "Akademik")
            <!-- button hapus -->
            <button type="button" data-modal-target="popup-modal-{{$show->_id}}" data-modal-toggle="popup-modal-{{$show->_id}}" class="px-3 py-1 bg-red-600 text-gray-50 font-bold rounded-lg mr-2 mb-5 hover:bg-gray-300">Hapus</button>
            @endif
            ...
            <h2 class="text-lg md:text-xl font-bold mb-2 mt-2">
                <a href="/daftar-tugas-akhir-detail/{{ $show->_id }}">
                    @php echo strtoupper($show->judul); @endphp
                </a>
            </h2>
        ...
        <!-- form hapus -->
            <form action="/destroy-tugas-akhir/{{$show->_id}}" method="POST">
                @csrf

                <!-- popup hapus -->
                <div id="popup-modal-{{$show->_id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative w-full max-w-md max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="p-6 text-center">
                                <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                <h3 class="mb-10 text-lg font-normal text-black dark:text-gray-400">Apakah kamu yakin ingin menghapus Tugas Akhir yang di pilih? <br><label class="font-bold text-red-600">Jika anda hapus maka Tugas Akhir tersebut akan di hapus secara permanen</label></h3>
                                <button data-modal-hide="popup-modal" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                    Yes, I'm sure
                                </button>
                                <button data-modal-hide="popup-modal-{{$show->_id}}" type="button" class="text-black bg-gray-100 hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
