...
@section('title', 'Daftar status ACC')

@section('content')
    <!-- search input -->
    <form action="{{ route('daftar-status-acc') }}" method="GET" class="flex items-center justify-center border-0 pt-5 pb-8">  
    ...
    <!-- isi content -->
    <div class="md:w-1/2 mt-5 md:mt-10 text-lg md:text-2xl font-bold mx-auto text-center text-blue-900">
        STORY DAFTAR USER DI ACC/TOLAK
    </div>
    <!-- tabel -->
    <div class="mt-6 overflow-x-auto">
        <table class="table-auto w-full bg-gray-50 shadow-md rounded-lg overflow-hidden">
            ...
            <tr class="bg-blue-900 font-bold text-green-300 uppercase text-sm leading-normal">
                ...
                <th class="py-3 px-6 text-center">Di ACC/Tolak Oleh</th>
        ...
        <!-- membuat nomor -->
        @php $no = ($getStory->currentPage() - 1) * $getStory->perPage() @endphp
        ...
        @foreach($getStory as $story)
            <tbody class="text-black text-sm font-medium">
                <tr class="border-b border-gray-200 hover:bg-green-100">
                    ...
                    <td class="py-3 px-6 text-left sm:whitespace-normal">
                        {{ $story->judul }}
                    </td>
                    ...
                    @if (auth()->user()->user_status == 'super admin')
                        <td class="py-3 px-6 text-left w-1/12">
                            <div class="flex justify-center">
                                <!-- form hapus -->
                                <form action="/hapus-daftar-status/{{$story->_id}}" method="POST">
                                    @csrf
                                    <button type="button" data-modal-target="popup-modal-{{$story->_id}}" data-modal-toggle="popup-modal-{{$story->_id}}" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Hapus</button>
                                    
                                    <!-- popup hapus -->
                                    <div id="popup-modal-{{$story->_id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative w-full max-w-md max-h-full">
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                <div class="p-6 text-center">
                                                    ...
                                                    <button data-modal-hide="popup-modal-{{$story->_id}}" type="button" class="text-black bg-gray-100 hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="flex flex-col items-center mt-8 mb-8">
            <span class="text-sm font-semibold text-gray-700 dark:text-gray-400">
                Showing <span class="font-bold">{{ $getStory->firstItem() }}</span> to <span class="font-bold">{{ $getStory->lastItem() }}</span> of <span class="font-bold">{{ $getStory->total() }}</span> entries
            </span>
            <div class="flex flex-wrap justify-center mt-4">
                @if ($getStory->onFirstPage())
                <span class="px-3 py-1 bg-gray-200 text-gray-500 rounded-lg mr-2 cursor-not-allowed">Prev</span>
                @else
                <a href="{{ $getStory->previousPageUrl() }}" class="px-3 py-1 bg-gray-200 text-gray-700 rounded-lg mr-2 hover:bg-gray-300">Prev</a>
                @endif

                @php
                    $start = 1;
                    $end = $getStory->lastPage();

                    if ($getStory->lastPage() > 5) {
                        $start = max($getStory->currentPage() - 2, 1);
                        $end = min($getStory->currentPage() + 2, $getStory->lastPage());

                        if ($start == 1) {
                            $end = min($end + abs($start - $getStory->currentPage()) , $getStory->lastPage());
                        }

                        if ($end == $getStory->lastPage()) {
                            $start = max($start - abs($end - $getStory->currentPage()), 1);
                        }
                    }
                @endphp

                @foreach ($getStory->getUrlRange($start, $end) as $page => $url)
                    @if ($page == $getStory->currentPage())
                ...
                @if ($getStory->hasMorePages())
                <a href="{{ $getStory->nextPageUrl() }}" class="px-3 py-1 bg-gray-200 text-gray-700 rounded-lg mr-2 hover:bg-gray-300">Next</a>
                @else
